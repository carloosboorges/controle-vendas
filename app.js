...
// ==========================================
// FUNÇÕES SEGURAS DE CACHE (ANTIFALHA)
// ==========================================
...
let authToken = getLS("vendas_auth_token");
let refreshToken = getLS("vendas_refresh_token");
let state = null;
let conexaoNuvemSucesso = false; // Trava de segurança para não apagar dados

let ocultarHistoricoTotal = getLS("putz_ocultar_historico") === "true";
let ocultarPeriodos = getLS("putz_ocultar_periodos") === "true";

function toggleOcultarHistorico() {
  ocultarHistoricoTotal = !ocultarHistoricoTotal;
  setLS("putz_ocultar_historico", ocultarHistoricoTotal);
  render();
}

function toggleOcultarPeriodos(e) {
  if (e) e.stopPropagation();
  ocultarPeriodos = !ocultarPeriodos;
  setLS("putz_ocultar_periodos", ocultarPeriodos);
  render();
}
...
try {
  if (window.BroadcastChannel) {
    syncChannel = new BroadcastChannel('putz_sync_channel');
    syncChannel.onmessage = (event) => {
      if (event.data === 'update' && currentUser) sincronizarSilencioso();
    };
  }
} catch(e) { console.warn("BroadcastChannel não suportado neste navegador."); }
...
function mudarAbaHistorico(aba) {
  ...
  const btnAgendamentos = document.getElementById("tabBtnAgendamentos");
  ...
  const divAgendamentos = document.getElementById("conteudoAbaAgendamentos");
  ...
  if (btnAgendamentos) btnAgendamentos.classList.remove("active");
  ...
  if (divAgendamentos) divAgendamentos.style.display = "none";
  ...
  if (aba === 'vendas') {
    ...
    if (btnAgendamentos) btnAgendamentos.classList.add("active");
    if (divAgendamentos) divAgendamentos.style.display = "block";
    if (typeof renderizarAgendamentos === 'function') renderizarAgendamentos();
...
document.addEventListener("click", (e) => {
  if (!e.target.closest(".period-card-calendar-container") && 
    ...
    if (dropCliente && dropCliente.style.display === "block") dropCliente.style.display = "none";
    
    const dropNick = document.getElementById("nickSuggestions");
    if (dropNick && dropNick.style.display === "block") dropNick.style.display = "none";

    const dropWpp = document.getElementById("whatsappSuggestions");
    if (dropWpp && dropWpp.style.display === "block") dropWpp.style.display = "none";

    const dropTk = document.getElementById("tiktokSuggestions");
    if (dropTk && dropTk.style.display === "block") dropTk.style.display = "none";
...
function lerObservacaoCliente(nome) {
   ...
   if (info.observacao) {
      const elNome = document.getElementById('leituraObsNome');
      const elTexto = document.getElementById('leituraObsTexto');
      const elModal = document.getElementById('leituraObsModal');
      if (elNome) elNome.textContent = "👤 Cliente: " + nome;
      if (elTexto) elTexto.textContent = info.observacao;
...
function buscarSugestoesCliente(texto) {
  ...
  dropdown.innerHTML = sugestoes.slice(0, 6).map(nome => {
    ...
    return `<div class="autocomplete-item" onclick="selecionarSugestaoCliente('${esc(nome).replace(/'/g, "\\'")}', '${esc(dados.nick).replace(/'/g, "\\'")}', '${esc(dados.whatsapp).replace(/'/g, "\\'")}', '${esc(dados.tiktok).replace(/'/g, "\\'")}')">
...
function buscarSugestoesNick(texto) {
  ...
  dropdown.innerHTML = sugestoes.slice(0, 6).map(nick => {
    ...
    return `<div class="autocomplete-item" onclick="selecionarSugestaoCliente('${esc(d.nome).replace(/'/g, "\\'")}', '${esc(nick).replace(/'/g, "\\'")}', '${esc(d.whatsapp).replace(/'/g, "\\'")}', '${esc(d.tiktok).replace(/'/g, "\\'")}')">
      🎮 ${esc(nick)} <span class="autocomplete-nick">👤 ${esc(d.nome)}</span>
    </div>`;
  }).join("");
  dropdown.style.display = "block";
}
...
function buscarSugestoesWhatsapp(texto) {
  ...
  dropdown.innerHTML = sugestoes.slice(0, 6).map(wpp => {
    ...
    const obsIcon = hasObs ? ' <span style="font-size:11px;" title="Possui observação">📌</span>' : '';
    return `<div class="autocomplete-item" onclick="selecionarSugestaoCliente('${esc(d.nome).replace(/'/g, "\\'")}', '${esc(d.nick).replace(/'/g, "\\'")}', '${esc(wpp).replace(/'/g, "\\'")}', '${esc(d.tiktok).replace(/'/g, "\\'")}')">
...
function buscarSugestoesTiktok(texto) {
  ...
  dropdown.innerHTML = sugestoes.slice(0, 6).map(tk => {
    ...
    const hasObs = state.clientesInfo && state.clientesInfo[d.nome] && state.clientesInfo[d.nome].observacao;
    const obsIcon = hasObs ? ' <span style="font-size:11px;" title="Possui observação">📌</span>' : '';
    return `<div class="autocomplete-item" onclick="selecionarSugestaoCliente('${esc(d.nome).replace(/'/g, "\\'")}', '${esc(d.nick).replace(/'/g, "\\'")}', '${esc(d.whatsapp).replace(/'/g, "\\'")}', '${esc(tk).replace(/'/g, "\\'")}')">
...
function selecionarSugestaoCliente(nome, nick, whatsapp, tiktok) {
  const elCliente = document.getElementById("clienteInput");
  const elNick = document.getElementById("nickClienteInput");
  const elWpp = document.getElementById("whatsappInput");
  const elTk = document.getElementById("tiktokInput");

  if (elCliente) elCliente.value = nome;
  if (elNick) elNick.value = nick;
  if (elWpp) elWpp.value = whatsapp || "";
  if (elTk) elTk.value = tiktok || "";
  
  const cSug = document.getElementById("clienteSuggestions");
  const nSug = document.getElementById("nickSuggestions");
  const wSug = document.getElementById("whatsappSuggestions");
  const tSug = document.getElementById("tiktokSuggestions");

  if (cSug) cSug.style.display = "none";
  if (nSug) nSug.style.display = "none";
  if (wSug) wSug.style.display = "none";
  if (tSug) tSug.style.display = "none";
...
function selecionarSugestaoNickPresente(nick, index) {
  ...
  const sug = document.getElementById(`nickPresenteSuggestions_${index}`);
...
function salvarRegistroApoiador() {
  const inputBruto = document.getElementById("apoiadorBrutoUsd");
  const inputLiquido = document.getElementById("apoiadorLiquidoBrl");
  const brutoUsd = parseFloat(inputBruto ? inputBruto.value : 0) || 0;
  const liquidoBrl = parseFloat(inputLiquido ? inputLiquido.value : 0) || 0;
            ...
            return `
              <tr>
                ...
                <td style="padding:12px; text-align:center; border-bottom:1px solid var(--border);">
                  <button type="button" class="btn-gray" style="padding:4px 8px; font-size:11px; margin-right:4px;" onclick="editarRegistroApoiador('${k}')">✏️ Editar</button>
...
function abrirModalAddConta() {
  const elNome = document.getElementById("novaContaNomeInput");
  const elVb = document.getElementById("novaContaVbucksInput");
  const elEmail = document.getElementById("novaContaEmailInput");
  const elSenha = document.getElementById("novaContaSenhaInput");
  const elModal = document.getElementById("addContaModal");
  
  if (elNome) elNome.value = "";
  if (elVb) elVb.value = "0";
...
function salvarNovaContaModal() {
  ...
  const vbucks = elVb ? parseInt(String(elVb.value || "0").replace(/\D/g, ""), 10) || 0 : 0;
...
function abrirModalEditConta(i) {
  ...
  const elIdx = document.getElementById("editContaIndex");
  const elNome = document.getElementById("editContaNomeInput");
  const elVb = document.getElementById("editContaVbucksInput");
  const elSomar = document.getElementById("editContaSomarVbucksInput");
  const elEmail = document.getElementById("editContaEmailInput");
  const elSenha = document.getElementById("editContaSenhaInput");
  const elModal = document.getElementById("editContaModal");

  if (elIdx) elIdx.value = i;
  if (elNome) elNome.value = conta.nome;
  if (elVb) elVb.value = Number(conta.vbucks) || 0;
  if (elSomar) elSomar.value = "";
  if (elEmail) elEmail.value = conta.email || "";
  if (elSenha) elSenha.value = conta.senha || "";
  if (elModal) elModal.style.display = "flex";
}
...
function aplicarSomaVbucksModal() {
  ...
  if (!somarInput || !saldoInput) return;
...
function salvarEdicaoContaModal() {
  const elIdx = document.getElementById("editContaIndex");
  if (!elIdx) return;
  const i = parseInt(elIdx.value, 10);
  const conta = state.contas[i];
  if (!conta) return;
  ...
  const novoNome = elNome ? elNome.value.trim() : "";
  const novoVbucks = elVb ? parseInt(String(elVb.value || "0").replace(/\D/g, ""), 10) : 0;
  const novoEmail = elEmail ? elEmail.value.trim() : "";
  const novaSenha = elSenha ? elSenha.value.trim() : "";
...
async function fazerLogin() {
  ...
  const elLembrar = document.getElementById("lembrarCredenciais");

  const email = elEmail ? elEmail.value.trim() : "";
  const password = elPass ? elPass.value.trim() : "";
  const lembrar = elLembrar ? elLembrar.checked : false;
  ...
  try {
    ...
    if (lembrar) {
      setLS("vendas_saved_email", email);
      setLS("vendas_saved_pass", password);
    }
    authToken = data.access_token;
    refreshToken = data.refresh_token || null;
    currentUser = data.user;
    
    setLS("vendas_auth_token", authToken);
    if (refreshToken) setLS("vendas_refresh_token", refreshToken);
...
function fazerLogout() {
  ...
  removeLS("vendas_auth_token");
  removeLS("vendas_refresh_token");
  fecharModalAuth();
  inicializar();
}

async function renovarTokenSupabase() {
  const storedRefresh = getLS("vendas_refresh_token");
...
async function verificarSessao() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const renovou = await renovarTokenSupabase();
      if (!renovou) { 
        currentUser = null; authToken = null; refreshToken = null; 
        removeLS("vendas_auth_token"); removeLS("vendas_refresh_token"); 
      }
    }
...
async function inicializar() {
  ...
  try {
    ...
    if (!res.ok) throw new Error("Erro na conexão HTTP");
    ...
    sanitizarDados();
    conexaoNuvemSucesso = true; // Liberou a trava de salvamento!
    
    if (footer) footer.textContent = `🟢 Conectado à Nuvem (Admin: ${currentUser.email})`;
  } catch (err) {
    conexaoNuvemSucesso = false; // Travou salvamento para não apagar nada!
    if (footer) footer.textContent = "⚠️ Erro crítico de conexão. Sistema em Modo de Segurança (Leitura).";
    state = JSON.parse(JSON.stringify(DADOS_DEMO));
    sanitizarDados();
    mostrarNotificacao("Sua conexão falhou e o sistema bloqueou o salvamento para proteger seus dados originais.", "erro");
  }
  render();
}
...
async function save() {
  ...
  if (!conexaoNuvemSucesso) {
    mostrarNotificacao("Ação bloqueada! O sistema não conseguiu sincronizar com a nuvem ao abrir a página.", "erro");
    return;
  }
...
function atualizarCamposItens() {
  const elQtd = document.getElementById("quantidadeInput");
  if (!elQtd) return;
  const qtd = parseInt(elQtd.value, 10) || 1;
...
function obterItensDaVenda() {
  const elQtd = document.getElementById("quantidadeInput");
  const qtd = elQtd ? parseInt(elQtd.value, 10) || 1 : 1;
  const lista = [];
  for (let i = 0; i < qtd; i++) {
    const elTipo = document.getElementById(`itemTypeSelect_${i}`);
    const elNome = document.getElementById(`itemNameInput_${i}`);
    const elPres = document.getElementById(`itemPresenteInput_${i}`);
    
    const tipo = elTipo ? elTipo.value : "Outro";
    const nome = elNome ? elNome.value.trim() : "";
    const presente = elPres ? elPres.value.trim() : "";
    if (nome) {
...
function render() {
  ...
  if (elTotalGeral) elTotalGeral.textContent = money(totalSessao);
  
  const qtdPedidosSessao = (state.vendas || []).length;
  const qtdItensSessao = (state.vendas || []).reduce((a, v) => a + (Number(v.quantidade) || 1), 0);
  const elQtdVendas = document.getElementById("qtdVendas");
  if (elQtdVendas) elQtdVendas.textContent = `${qtdPedidosSessao} (${qtdItensSessao} itens)`;

  let top = "—", tv = 0;
  Object.entries(tSessao).forEach(([n, v]) => { if (v > tv) { top = n; tv = v; } });
  
  const elTopConta = document.getElementById("topConta");
  if (elTopConta) elTopConta.textContent = tv ? `${top} — ${money(tv)}` : "—";
  ...
  const elContas = document.getElementById("contas");
  if (elContas) {
    elContas.innerHTML = (state.contas || []).map((c, i) => `
  ...
  const elHistQtd = document.getElementById("historicoQtdTotal");
  if (elHistQtd) elHistQtd.textContent = `${totalPedidosHistorico} pedidos · ${totalItensHistorico} itens enviados`;

  // ----- RENDERIZAÇÃO DOS OLHINHOS MÁGICOS -----
  const elHistTotal = document.getElementById("historicoTotal");
  if (elHistTotal) {
    const eyeIconHist = ocultarHistoricoTotal ? "🙈" : "👁️";
    const historicoFormatado = ocultarHistoricoTotal ? "R$ •••••" : money(totalHistorico);
    elHistTotal.innerHTML = `Total do histórico: ${historicoFormatado} <span style="cursor:pointer; margin-left:8px; font-size:20px; vertical-align:middle;" onclick="toggleOcultarHistorico()" title="Ocultar/Mostrar Valor">${eyeIconHist}</span>`;
  }

  const elLixeiraBtn = document.getElementById("lixeiraBtn");
  if (elLixeiraBtn) elLixeiraBtn.textContent = `🗑️ Lixeira (${(state.lixeiraVendas || []).length})`;
  ...
  if (periodosEl) {
    ...
    const vPer = (v) => ocultarPeriodos ? '<span style="letter-spacing: 2px;">•••••</span>' : money(v);
    const eyePer = `<span style="cursor:pointer; font-size:16px; margin-left:auto;" onclick="toggleOcultarPeriodos(event)" title="Ocultar/Mostrar Valores">${ocultarPeriodos ? '🙈' : '👁️'}</span>`;
    ...
    <div class="period-card period-card-calendar-container" style="position:relative; cursor:pointer;" onclick="toggleCalendarioPopover(event)">
      <div class="period-header-select" style="display:flex; align-items:center;">
        <span>📅</span>
        <strong style="font-size:12px; color:var(--accent-light); margin-left:6px;">${isModoHoje ? `Hoje (${diaParaFiltrar.slice(0, 5)})` : diaParaFiltrar} ▾</strong>
        ${eyePer}
      </div>
      <strong>${vPer(diaTotal)}</strong>
    ...
    <div class="period-card">
      ...
      <strong>${vPer(semanaTotal)}</strong>
    ...
    <div class="period-card period-card-mes-container" style="position:relative; cursor:pointer;" onclick="toggleMesPopover(event)">
      <div class="period-header-select" style="display:flex; align-items:center;">
        <span>🗓️</span>
        <strong style="font-size:12px; color:var(--accent-light); margin-left:6px;">${nomeMesSelecionadoLabel} ▾</strong>
        ${eyePer}
      </div>
      <strong>${vPer(mesTotalVendas)}</strong>
      ...
      <div style="font-size:10px; margin-top:4px; border-top:1px solid rgba(255,255,255,0.06); padding-top:4px; line-height:1.3;">
        <span style="color:var(--muted);">Vendas:</span> ${vPer(lucroVendasMes)}<br>
        <span style="color:var(--muted);">Apoiador:</span> ${vPer(lucroApoiadorMes)}<br>
        <strong style="color:var(--green);">Total: ${vPer(lucroTotalMesCombinado)}</strong>
      </div>
    ...
    <div class="period-card period-card-ano-container" style="position:relative; cursor:pointer;" onclick="toggleAnoPopover(event)">
      <div class="period-header-select" style="display:flex; align-items:center;">
        <span>📆</span>
        <strong style="font-size:12px; color:var(--accent-light); margin-left:6px;">Ano ${anoFiltroSelecionado} ▾</strong>
        ${eyePer}
      </div>
      <strong>${vPer(anoTotalVendas)}</strong>
      <small>${anoPedidos} pedidos (${anoItens} itens)</small>
      <small class="period-vbucks-text">🪙 ${formatVBucks(anoVbucks)} V-Bucks</small>
      <div style="font-size:10px; margin-top:4px; border-top:1px solid rgba(255,255,255,0.06); padding-top:4px; line-height:1.3;">
        <span style="color:var(--muted);">Vendas:</span> ${vPer(lucroVendasAno)}<br>
        <span style="color:var(--muted);">Apoiador:</span> ${vPer(lucroApoiadorAno)}<br>
        <strong style="color:var(--green);">Total: ${vPer(lucroTotalAnoCombinado)}</strong>
      </div>
    ...
    <div class="period-card profit-card">
      ...
      <strong>${vPer(lucroLiquidoGlobalTotal)}</strong>
      <small style="color:var(--green); font-weight:700;">Vendas: ${vPer(lucroContinuoGeralVendas)}</small>
      <small style="color:var(--accent-light); font-weight:700;">Apoiador: ${vPer(totalLiquidoApoiadorGlobal)}</small>
    </div>
  `;
  }
  ...
  if (abaHistoricoAtiva === 'agendamentos' && typeof renderizarAgendamentos === 'function') {
    renderizarAgendamentos();
  } else if (abaHistoricoAtiva === 'apoiador') {
    renderizarHistoricoApoiadorCompleto();
...
function editarRegistroApoiador(chaveMes) {
  apoiadorMesSelecionadoTemp = chaveMes;
  const [m, a] = chaveMes.split("/").map(Number);
  apoiadorPopoverAno = a;
  atualizarModalApoiadorHTML();
  document.getElementById("apoiadorModal").style.display = "flex";
}

function removerRegistroApoiadorCompleto(chave) {
  abrirModalConfirmacao("🗑️ Excluir Apoiador", `Tem certeza que deseja excluir o registro de ${chave}?`, () => {
    delete state.apoiadorRegistros[chave];
    save();
    renderizarHistoricoApoiadorCompleto();
    mostrarNotificacao(`Registro de ${chave} excluído.`, "info");
  });
}
...
function agendarVenda() {
  ...
  const elWpp = document.getElementById("whatsappInput");
  const elTk = document.getElementById("tiktokInput");
  const elObs = document.getElementById("observacaoInput");
  const elQtd = document.getElementById("quantidadeInput");

  const conta = elConta ? elConta.value : "";
  const valor = elValor ? parseFloat(elValor.value) : 0;
  const cliente = elCliente ? elCliente.value.trim() : "";
  const nickCliente = elNick ? elNick.value.trim() : "";
  const whatsapp = elWpp ? elWpp.value.trim() : "";
  const tiktok = elTk ? elTk.value.trim() : "";
  const observacao = elObs ? elObs.value.trim() : "";
  const quantidade = elQtd ? parseInt(elQtd.value, 10) || 1 : 1;
  ...
  const btnLimpar = document.getElementById("limparTudoBtn");
...
function abrirModalEdicaoAgendamento(id) {
  ...
  const elVendaId = document.getElementById("editVendaId");
  const elTitle = document.getElementById("editSaleTitle");
  if(elVendaId) elVendaId.value = id;
  if(elTitle) elTitle.innerHTML = "✏️ Editar Pré-venda Agendada";
  ...
  const elCliente = document.getElementById("editClientInput");
  const elNick = document.getElementById("editNickInput");
  const elWpp = document.getElementById("editWhatsappInput");
  const elTk = document.getElementById("editTiktokInput");
  const elObs = document.getElementById("editObservacaoInput");
  const elData = document.getElementById("editDataInput");
  const elHora = document.getElementById("editHoraInput");
  const elValor = document.getElementById("editValorInput");

  if(elCliente) elCliente.value = agendamento.cliente || "";
  if(elNick) elNick.value = agendamento.nickCliente || "";
  if(elWpp) elWpp.value = agendamento.whatsapp || "";
  if(elTk) elTk.value = agendamento.tiktok || "";
  if(elObs) elObs.value = agendamento.observacao || "";
  if(elData) elData.value = agendamento.dataRegistro || "";
  if(elHora) elHora.value = agendamento.horaRegistro || "";
  if(elValor) elValor.value = Number(agendamento.valor || 0).toFixed(2);
  ...
  const elModal = document.getElementById("editSaleModal");
  if(elModal) elModal.style.display = "flex";
}
...
// ==========================================
// REGISTRO SEGURO DOS BOTÕES E IGNIÇÃO
// ==========================================
document.addEventListener("DOMContentLoaded", () => {
  const btnLimpTudo = document.getElementById("limparTudoBtn");
  if (btnLimpTudo) {
    btnLimpTudo.addEventListener("click", () => {
      const elVal = document.getElementById("valorInput");
      const elCli = document.getElementById("clienteInput");
      const elNick = document.getElementById("nickClienteInput");
      const elWpp = document.getElementById("whatsappInput");
      const elTk = document.getElementById("tiktokInput");
      const elObs = document.getElementById("observacaoInput");
  ...
  const btnLimpVal = document.getElementById("limparSoValorBtn");
  if (btnLimpVal) {
    btnLimpVal.addEventListener("click", () => {
      const elVal = document.getElementById("valorInput");
      if (elVal) {
        elVal.value = "";
        atualizarPreviewVBucks();
