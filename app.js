...
// ==========================================
// VARIÁVEIS DE PRIVACIDADE (OLHINHO MÁGICO)
// ==========================================
let ocultarHistoricoTotal = localStorage.getItem("putz_ocultar_historico") === "true";
let ocultarPeriodos = localStorage.getItem("putz_ocultar_periodos") === "true";

function toggleOcultarHistorico() {
  ocultarHistoricoTotal = !ocultarHistoricoTotal;
  localStorage.setItem("putz_ocultar_historico", ocultarHistoricoTotal);
  render();
}
...
function toggleOcultarPeriodos(e) {
  ...
  localStorage.setItem("putz_ocultar_periodos", ocultarPeriodos);
  render();
}
...
const syncChannel = window.BroadcastChannel ? new BroadcastChannel('putz_sync_channel') : null;
if (syncChannel) {
...
function buscarSugestoesNick(texto) {
  ...
  histReverso.forEach(v => {
     ...
     if(nick && !nicksMap[nick]) {
         nicksMap[nick] = { nome: v.cliente || "", whatsapp: v.whatsapp || "", tiktok: v.tiktok || "" };
     }
  });
  ...
  dropdown.innerHTML = sugestoes.slice(0, 6).map(nick => {
    ...
    return `
    <div class="autocomplete-item" onclick="selecionarSugestaoCliente('${esc(d.nome).replace(/'/g, "\\'")}', '${esc(nick).replace(/'/g, "\\'")}', '${esc(d.whatsapp).replace(/'/g, "\\'")}', '${esc(d.tiktok).replace(/'/g, "\\'")}')">
...
async function inicializar() {
  ...
  try {
    ...
    if (footer) footer.textContent = "⚠️ Erro crítico de conexão. Sistema em Modo de Segurança.";
    state = JSON.parse(JSON.stringify(DADOS_DEMO));
    sanitizarDados();
    render();
  }
...
function render() {
  ...
  if (elHistTotal) {
    ...
    elHistTotal.innerHTML = `Total do histórico: ${historicoFormatado} <span style="cursor:pointer; margin-left:8px; font-size:20px; vertical-align:middle; filter:drop-shadow(0 2px 4px rgba(0,0,0,0.3));" onclick="toggleOcultarHistorico()" title="Ocultar/Mostrar Valor">${eyeIconHist}</span>`;
  }
